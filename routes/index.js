...
router.post('/estimate/:origins/:destinations/:arrivalTime', async (req, res, next) => {
  ...
  try {
    ...
    const originToFirstRestAreaResponse = await axios.get(originToFirstRestAreaUrl);
    const originToFirstRestArea = originToFirstRestAreaResponse.data.rows[0].elements[0];
    const durationInSeconds = originToFirstRestArea.duration.value;

    const arrivalTime = moment(departureTime).add(durationInSeconds, 'seconds');
    ...
    for (let i = 0; i < restAreas.length - 1; i++) {
      ...
      const arrivalTime = moment(departureTime).add(durationInSeconds, 'seconds').format(); // Calculate arrival time
    ...
    const lastRestAreaToDestinationUrl = `https://api.distancematrix.ai/maps/api/distancematrix/json?origins=${restAreas[restAreas.length - 1]}&destinations=${destinations}&key=${key}`;
    const lastRestAreaToDestinationResponse = await axios.get(lastRestAreaToDestinationUrl);
    const lastRestAreaToDestination = lastRestAreaToDestinationResponse.data.rows[0].elements[0];

    const departureTimeLast = previousArrivalTime.format();
    const durationInSecondsLast = lastRestAreaToDestination.duration.value;
    const arrivalTimeLast = moment(departureTimeLast).add(durationInSecondsLast, 'seconds').format();
