...
const fetch_distance = async (origin, destination) => {
    let result = await axios.get(`https://api.distancematrix.ai/maps/api/distancematrix/json?origins=${origin}&destinations=${destination}&mode=driving&key=wNKvwwrlvWheyBOd84pP8uIsbqhW1`)
    return result;
}
...
const estimateTimeTwo = async (trips_arr, base_addr, multiplier, stop_time) => {
    ...
    for (let i = 0; i < pts.length; i++) {
        ...
        let duration_1 = await fetch_distance(pts[i].doc.origin_point_adress, base_address)


        if (i < pts.length - 1) {

            let first_step = await fetch_distance(pts[i].doc.origin_point_adress, pts[i + 1].doc.origin_point_adress)
            let second_step = await fetch_distance(pts[i + 1].doc.origin_point_adress, base_address)
    ...
    const newData = estimates.map(obj => {
        ...
        let first_step_with_multiplier = Number(firstStep.duration.value) * Number(multiplier)
        let second_step_step_with_multiplier = Number(secondStep.duration.value) * Number(multiplier)

        // Calculate total time in seconds
        let totalTime = Number(first_step_with_multiplier) + Number(second_step_step_with_multiplier);

        //duration_1 time with drop time added
        let duration_1_drop_time_added = Number(duration_1.duration.value) + Number(stop_time)
